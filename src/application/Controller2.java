...
public class Controller2 implements Initializable{
	...
	public void RunOnAction(ActionEvent e) {
			...
			if (combo1.getSelectionModel().getSelectedItem().toString().contains(c.al.get(i).getName())) {
				for (int j = 0; j < c.al.size(); j++) {
					if (combo2.getSelectionModel().getSelectedItem().toString().contains(c.al.get(j).getName())) {
						if(UCS.isSelected()) {
							if(graph.UniformCostSearch(c.al.get(i), c.al.get(j))!=Integer.MAX_VALUE) {
								textfield.setText(Double.toString(graph.UniformCostSearch(c.al.get(i), c.al.get(j))));
								String[] arr = graph.path(c.al.get(j));
								arrl.add(arr);
								counter1=graph.UniformCostSearch(c.al.get(i), c.al.get(j));
							}else{
							
								counter1=graph.UniformCostSearch(c.al.get(i), c.al.get(j));
							}
						}else if(Astar.isSelected()) {
							if(graph.AStarSearch(c.al.get(i), c.al.get(j))!=Integer.MAX_VALUE) {
								textfield.setText(Double.toString(graph.AStarSearch(c.al.get(i), c.al.get(j))));
								String[] arr = graph.path(c.al.get(j));
								arrl.add(arr);
								counter1=graph.AStarSearch(c.al.get(i), c.al.get(j));
							}else{
		...
		Time.setText(""+graph.getTimeCount());
		Space.setText(""+graph.getSizeCount());
	...
	public void ClearOnAction(ActionEvent e) {
		...
		int counter1 = (pane2.getChildren().size())-51;
		if(pane2.getChildren().size()!=51) {
